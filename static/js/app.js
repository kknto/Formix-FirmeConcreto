...
const remFilterDate = document.getElementById("remFilterDate");
const remSearchInput = document.getElementById("remSearchInput");
const remReloadBtn = document.getElementById("remReloadBtn");
const dRemisionDate = document.getElementById("dRemisionDate");
...
function applyRoleAccessUi() {
  ...
  if (tabRemisiones) tabRemisiones.style.display = (canAccessView("dosificador") || canAccessView("remisiones")) ? "" : "none";
...
function switchView(view) {
  ...
  const isRem = view === "remisiones";
  const isUser = view === "usuarios";
  ...
  if (remisionesView) remisionesView.classList.toggle("is-hidden", !isRem);
  if (usuariosView) usuariosView.classList.toggle("is-hidden", !isUser);
  ...
  if (tabRemisiones) tabRemisiones.classList.toggle("view-tab--active", isRem);
  if (tabUsuarios) tabUsuarios.classList.toggle("view-tab--active", isUser);
...
async function loadRemisiones() {
  if (!canAccessView("dosificador") && !canAccessView("remisiones")) return;
  try {
    const isRemisionesView = state.view === "remisiones";
    const dateVal = isRemisionesView
      ? (remFilterDate?.value || "")
      : (dRemisionDate?.value || "");
    const qVal = isRemisionesView ? (remSearchInput?.value || "") : "";
    const limit = isRemisionesView ? 150 : 80;

    let url = `/api/remisiones?file=${encodeURIComponent(state.file || "")}&limit=${limit}`;
    ...
    if (isRemisionesView) {
      renderHistoricalRemisiones(payload.items || []);
    } else {
      state.doser.remisiones = Array.isArray(payload.items) ? payload.items : [];
      renderRemisionList();
    }
  } catch (error) {
    if (state.view === "remisiones") {
      remisionesBody.innerHTML = `<tr><td colspan="8">Error: ${String(error)}</td></tr>`;
    } else {
...
function renderHistoricalRemisiones(items) {
  if (!remisionesBody) return;
  remisionesBody.innerHTML = "";
  if (!items.length) {
    remisionesBody.innerHTML = `<tr><td colspan="8">No se encontraron remisiones con los filtros aplicados.</td></tr>`;
    return;
  }
  items.forEach((item) => {
    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td><strong>${escapeHtml(item.remision_no || "-")}</strong></td>
      <td>${escapeHtml(item.formula || "-")}</td>
      <td>${escapeHtml(item.fc || "-")}</td>
      <td>${formatNum(item.dosificacion_m3 || 0)} m³</td>
      <td>${formatNum(item.peso_real_total || 0)} kg</td>
      <td>${escapeHtml(item.created_at || "-")}</td>
      <td>${escapeHtml(item.created_by || "-")}</td>
      <td>
        <button type="button" class="btn btn--secondary btn--small" onclick="AppGlobals.openRemisionReport(${item.id})">Ver / Imprimir</button>
      </td>
    `;
    remisionesBody.appendChild(tr);
  });
}
...
if (dRemisionDate) dRemisionDate.addEventListener("change", loadRemisiones);

if (remReloadBtn) remReloadBtn.addEventListener("click", loadRemisiones);
if (remFilterDate) remFilterDate.addEventListener("change", loadRemisiones);
if (remSearchInput) {
  remSearchInput.addEventListener("input", () => {
    // Optional: add debounce if performance is an issue
    loadRemisiones();
  });
}
if (remisionNoInput) {
